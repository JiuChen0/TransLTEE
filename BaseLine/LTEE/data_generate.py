...
for i in range(1,11):
    TY=np.loadtxt('BaseLine/LTEE/data/NEWS/topic_doc_mean_n5000_k3477_seed_'+str(i)+'.csv.y',delimiter=',')
    treatment=TY[:,0]
    ...
    with open('BaseLine/LTEE/data/NEWS/topic_doc_mean_n5000_k3477_seed_'+str(i)+'.csv.x', 'r') as fin:
    ...
    #print causal_effects
    np.savetxt('BaseLine/LTEE/data/NEWS/Series_groundtruth_'+str(i)+'.txt', causal_effects, delimiter=',', fmt='%.2f')
    np.savetxt('BaseLine/LTEE/data/NEWS/Series_y_'+str(i)+'.txt', data, delimiter=',', fmt='%.2f')
