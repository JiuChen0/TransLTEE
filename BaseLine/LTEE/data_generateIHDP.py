...
for i in range(1,11):
    TY=np.loadtxt('BaseLine/LTEE/data/IHDP/csv/ihdp_npci_'+str(i)+'.csv',delimiter=',')
    treatment=TY[:,0]
    ...
    np.savetxt('BaseLine/LTEE/data/IHDP/Series_groundtruth_'+str(i)+'.txt', causal_effects, delimiter=',', fmt='%.2f')
    np.savetxt('BaseLine/LTEE/data/IHDP/Series_y_'+str(i)+'.txt', data, delimiter=',', fmt='%.2f')
