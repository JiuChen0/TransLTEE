...
class cfr_net(object):
    ...
    def _build_graph(self, x, t, y_ , p_t, FLAGS, r_alpha, r_lambda, do_in, do_out, dims):
        ...
        self.phi=tf.keras.layers.Dense(dim_in, activation=tf.nn.relu, kernel_regularizer=regularizer)(x)
        
        ''' Regularization '''
        self.wd_loss+=tf.compat.v1.losses.get_regularization_loss()
    ...
    def attention(self, inputs, attention_size, time_major=False, return_alphas=False):
        ...
        # print(inputs.shape) 输入input为（None,400）只有2个维度
        hidden_size = inputs.shape[2]  # D value - hidden size of the RNN layer

        # Trainable parameters
        w_omega=self._create_variable(tf.compat.v1.random_normal([hidden_size, attention_size],
                                                              stddev=FLAGS.weight_init / np.sqrt(hidden_size)), 'w_omega')
        b_omega=self._create_variable(tf.compat.v1.random_normal([attention_size],
                                                              stddev=FLAGS.weight_init / np.sqrt(attention_size)), 'b_omega')
        u_omega = self._create_variable(tf.compat.v1.random_normal([attention_size],
    ...
    def _build_output(self, phi, rep_fw,rep_bw, dim_in, dim_out, do_out, FLAGS):
        ...
        h_input = tf.concat([rep_fw, rep_bw], axis=2)  # 在第二维度上连接 时间步长维度？
        ...
        y_seq = tf.keras.layers.Dense(1, activation=tf.nn.relu)(h_input)
        ...
        weights_pred = self._create_variable(tf.compat.v1.random_normal([dim_out+dim_in, 1],
    ...
    def _build_output_graph(self, rnn_outputs, t, dim_in, dim_out, do_out, FLAGS):
        ...
        i0 = tf.compat.v1.to_int32(tf.where(t < 1)[:,0])
        i1 = tf.compat.v1.to_int32(tf.where(t > 0)[:,0])
